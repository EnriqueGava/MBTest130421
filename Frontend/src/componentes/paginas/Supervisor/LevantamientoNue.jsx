...
export default function Nue() {
  ...
  React.useEffect(() => {
    ...
    axios.post("/users/user", {token : Cookie.get("Token")})
    .then(data => {
      axios.get(`/levantamientos/listarun/${data.data}`)
        .then(result => {
...
const handleFirmaClose = () => {
  ...
  const Signed =() => {
    FirmaSup(user.Supervisor_Id,imageData);
	console.log("Guardando firma");
	LevSup(levantamiento.Id_numorden,user.Supervisor_Id);
  ...
  const CrearL = async () => {
    ...
    setAcordion(true);    
    setDL(false);
    NuevoLevantamiento(user.User_ID);
  ...
  const checkFirma = async() => {
		let q= await checkSup(user.User_ID);
		//console.log(q.firma);
        if( q.firma == null)
			handleFirmaOpen();
		else{
      await GenerarL(levantamiento.Id_numorden);
      await LevSup(levantamiento.Id_numorden,user.Supervisor_Id);
      swal("Levantamiento Generado", Cookie.get("Nombre"), "success");
      setInterval(function () {
